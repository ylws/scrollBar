...
* 日期：2017/5/2
*/
var shineonScroll = function () {
	function shineonScroll(opt) {
		_classCallCheck(this, shineonScroll);

		this.initVal(opt);
	}

	_createClass(shineonScroll, [{
		key: "initVal",
		value: function initVal(opt) {
			...
			var defaultinitConfig = {
				...
				"wid_scrollwidth": 0 };
			this.cfg = {};
			_extends(this.cfg, defaultConfig, defaultinitConfig, opt);
			...
			if (this.getId(this.cfg.father).style.display == "none") {
				// 如果一开始为隐藏，则不再进行下面的操作,放置有的模拟滚动条默认为隐藏时，获取不到offset属性
				return false;
			} else {
				this.offsetval();
				this.init();
		...
		key: "getIdClass",
		value: function getIdClass(id, classname) {
			return document.getElementById(id).querySelector('.' + classname);
		}
	}, {
		key: "wheely",
		value: function wheely() {
			//执行一次mousemove事件
		...
		value: function wheelx() {
			...
			//计算滚动位置，子元素移动多长距离
			var minstep = (this.cfg.wid_soncontent - this.cfg.wid_father) / this.cfg.wid_e_s_wid;
		...
		value: function scrollings() {
			...
			for (var i = 0; i < soncobj.length; i++) {
				soncobj[i].onmouseover = function () {
					...
					var fatherhei = _this.getId(idval).clientHeight;
					var fatherwid = _this.getId(idval).clientWidth;
		...
		value: function onmouseclick() {
			if (!navigator.userAgent.match(/(iPhone|Android|iPad)/i)) {
				...
				this.getId(sf).querySelector("." + this.cfg.scroll_y).onmousedown = function (e) {
					...
					flag = true;
					_this2.cfg.hei_nowposition_y = e.pageY || e.clientY; //获取当前点击点的位置
					//ie、chrome当top为0时，值为auto,需做处理
					var styletop = this.style.top;
					if (styletop == "auto") {
						styletop = '0px';
					}
					_this2.cfg.hei_scroll_y_height = parseInt(styletop);
					//前点击点距离顶部的位置
					_this2.cfg.hei_click_top = _this2.cfg.hei_nowposition_y - _this2.cfg.hei_f_offhei - _this2.cfg.hei_scroll_y_height;
					if (flag) {
						...
						document.onmousemove = function (e) {
							...
							//y轴
							_this2.cfg.hei_scroll_y_height = parseInt(_this2.getIdClass(_this2.cfg.father, _this2.cfg.scroll_y).style.top);
							_this2.cfg.hei_nowposition_y_up = e.pageY || e.clientY; //获取移动点的坐标
							_this2.cfg.hei_scrolltop_y = _this2.cfg.hei_nowposition_y_up - _this2.cfg.hei_f_offhei - _this2.cfg.hei_click_top;
							_this2.cfg.hei_e_s_y_hei = _this2.cfg.hei_father - _this2.cfg.hei_scrolly;
							_this2.cfg.hei_soncontent = _this2.getIdClass(_this2.cfg.father, _this2.cfg.soncontent).clientHeight; //鼠标移动到两个同时存在的模拟滚动条时，会被覆盖掉，需重置
							_this2.cfg.hei_father = _this2.getId(_this2.cfg.father).clientHeight; //鼠标移动到两个同时存在的模拟滚动条时，会被覆盖掉，需重置
							//计算滚动位置，子元素移动多长距离
							_this2.cfg.hei_scrollheight = _this2.cfg.hei_scroll_y_height * ((_this2.cfg.hei_soncontent - _this2.cfg.hei_father) / _this2.cfg.hei_e_s_y_hei);
							if (_this2.cfg.hei_scrolltop_y <= 0) {
								_this2.getIdClass(_this2.cfg.father, _this2.cfg.scroll_y).style.top = '0px';
								_this2.getIdClass(_this2.cfg.father, _this2.cfg.soncontent).style.marginTop = '0px';
							} else if (_this2.cfg.hei_scrolltop_y >= _this2.cfg.hei_e_s_y_hei) {
								_this2.getIdClass(_this2.cfg.father, _this2.cfg.scroll_y).style.top = _this2.cfg.hei_e_s_y_hei + "px";
								_this2.getIdClass(_this2.cfg.father, _this2.cfg.soncontent).style.marginTop = -(_this2.cfg.hei_soncontent - _this2.cfg.hei_father) + 'px';
								if (window[_this2.smsscrollbottomfn]) {
									window[_this2.smsscrollbottomfn](_this2);
								}
							} else {
								_this2.getIdClass(_this2.cfg.father, _this2.cfg.scroll_y).style.top = _this2.cfg.hei_scrolltop_y + "px";
								_this2.getIdClass(_this2.cfg.father, _this2.cfg.soncontent).style.marginTop = -_this2.cfg.hei_scrollheight + 'px';
							}
						};
					}
				};
				...
				this.getId(sf).querySelector("." + this.cfg.scroll_x).onmousedown = function (e) {
					...
					flag = true;
					_this2.cfg.wid_nowposition_x = e.pageX || e.clientX; //获取当前点击点的位置
					//ie、chrome当left为0时，值为auto,需做处理
					var styleleft = this.style.left;
					if (styleleft == "auto" || styleleft == '') {
						styleleft = '0px';
					}
					_this2.cfg.wid_scroll_x_width = parseInt(styleleft);
					//前点击点距离顶部的位置
					_this2.cfg.wid_click_left = _this2.cfg.wid_nowposition_x - _this2.cfg.wid_f_offwid - _this2.cfg.wid_scroll_x_width;
					if (flag) {
						document.onmousemove = function (e) {
							...
							//x轴
							_this2.cfg.wid_scroll_x_width = parseInt(_this2.getIdClass(sf, _this2.cfg.scroll_x).style.left);
							_this2.cfg.wid_np_x_left = e.pageX || e.clientX; //获取移动点的坐标
							_this2.cfg.wid_scrollleft_x = _this2.cfg.wid_np_x_left - _this2.cfg.wid_f_offwid - _this2.cfg.wid_click_left;
							_this2.cfg.wid_e_s_wid = _this2.cfg.wid_father - _this2.cfg.wid_scrollx;
							_this2.cfg.wid_soncontent = _this2.getIdClass(sf, _this2.cfg.soncontent).clientWidth; //鼠标移动到两个同时存在的模拟滚动条时，会被覆盖掉，需重置
							_this2.cfg.wid_father = _this2.getId(sf).clientWidth; //鼠标移动到两个同时存在的模拟滚动条时，会被覆盖掉，需重置
							//计算滚动位置，子元素移动多长距离
							_this2.cfg.wid_scrollwidth = _this2.cfg.wid_scroll_x_width * ((_this2.cfg.wid_soncontent - _this2.cfg.wid_father) / _this2.cfg.wid_e_s_wid);
							if (_this2.cfg.wid_scrollleft_x <= 0) {
								_this2.getIdClass(sf, _this2.cfg.scroll_x).style.left = '0px';
								_this2.getIdClass(sf, _this2.cfg.soncontent).style.marginLeft = '0px';
							} else if (_this2.cfg.wid_scrollleft_x >= _this2.cfg.wid_e_s_wid) {
								_this2.getIdClass(sf, _this2.cfg.scroll_x).style.left = _this2.cfg.wid_e_s_wid + "px";
								_this2.getIdClass(sf, _this2.cfg.soncontent).style.marginLeft = -(_this2.cfg.wid_soncontent - _this2.cfg.wid_father) + "px";
								if (window[_this2.smsscrollbottomfn]) {
									window[_this2.smsscrollbottomfn](_this2);
								}
							} else {
								_this2.getIdClass(sf, _this2.cfg.scroll_x).style.left = _this2.cfg.wid_scrollleft_x + "px";
								_this2.getIdClass(sf, _this2.cfg.soncontent).style.marginLeft = -_this2.cfg.wid_scrollwidth + "px";
							}
						};
					}
				};
				document.onmouseup = function () {
					...
					if (window[this.smscrollfn]) {
						window[this.smscrollfn](sf, _this2.cfg.wheelval);
					}
				};
			}
		}
		...
		key: "scrollFunc",
		value: function scrollFunc(that) {
			return function (e) {
				var idval = "";
				...
				var sonhei = that.getIdClass(that.cfg.father, that.cfg.soncontent).clientHeight;
				var sonwid = that.getIdClass(that.cfg.father, that.cfg.soncontent).clientWidth;
				...
				if (fatherhei < sonhei && that.cfg.wheelxory == "wheely" || fatherhei > sonhei && that.cfg.wheelxory == "wheely" && top.location.href != location.href || fatherwid < sonwid && that.cfg.wheelxory == "wheelx" || fatherwid > sonwid && that.cfg.wheelxory == "wheelx" && top.location.href != location.href) {
					...
					if (document.getElementById(sf).offsetTop != undefined) {
						fathery = that.getId(sf).offsetTop;
						fatherx = that.getId(sf).offsetLeft;
						if (funx == undefined) {
							funx = fatherx;
						}
						if (funy == undefined) {
